...
export default function ChairpersonWithdrawalApprovals() {
  const { user } = useAuth();
  ...
  // Fetch withdrawal requests for chairperson
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Get withdrawal requests where user is a chairperson signatory
        ...
        // Filter to only show withdrawals where user is chairperson signatory
        const filteredWithdrawals = withdrawalsData?.filter((w: any) => {
          return w.withdrawal_signatories?.some(
            (s: any) => s.signatory_role === 'chairperson' && (!s.signatory_user_id || s.signatory_user_id === user.id)
          );
        }) || [];
  ...
  // Get pending withdrawals for chairperson
  const getPendingWithdrawals = (): WithdrawalRequest[] => {
    return withdrawals.filter((w) => {
      const chairpersonSignatory = w.signatories?.find(
        (s) => s.signatory_role === 'chairperson' && s.status === 'pending'
      );
