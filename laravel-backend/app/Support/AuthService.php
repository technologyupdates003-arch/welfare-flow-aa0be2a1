...
final class AuthService
{
    ...
    public static function identity(): Identity
    {
        ...
        $user = Db::one('SELECT id, email, is_banned FROM auth_users WHERE id = ?', [$userId]);
        if ($user === null) {
            throw new ApiException(401, 'user_not_found', 'Account no longer exists');
        }
    ...
    {
        $row = Db::one('SELECT id FROM members WHERE user_id = ? LIMIT 1', [$userId]);
    ...
    public static function issueSession(array $user): array
    {
        ...
        Db::run(
            ...
            [Uuid::v4(), $userId, $refresh, gmdate('Y-m-d H:i:s', time() + (int) config('welfare.refresh_ttl'))]
        );

        Db::run('UPDATE auth_users SET last_sign_in_at = UTC_TIMESTAMP(3) WHERE id = ?', [$userId]);

        $member = Db::one(
            'SELECT id, name, phone, status, is_active FROM members WHERE user_id = ? LIMIT 1',
            [$userId]
        );
        ...
        return [
            ...
            'token_type' => 'bearer',
            'expires_in' => (int) config('welfare.jwt_ttl'),
