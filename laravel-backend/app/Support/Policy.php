...
final class Policy
{
    ...
    public static function rules(string $table): array
    {
        if (!isset(self::TABLES[$table])) {
            throw new ApiException(404, 'unknown_table', sprintf('Table "%s" is not exposed by the API', $table));
        }
    ...
    public static function readScope(string $table, Identity $identity): ?array
    {
        ...
        $own = $rules['own'] ?? null;
        if ($own === null) {
            throw new ApiException(403, 'forbidden', sprintf('You are not allowed to read "%s"', $table));
        }
        if ($own === 'user_id') {
            return ['`user_id` = ?', [$identity->userId]];
        }
        if ($identity->memberId === null) {
            throw new ApiException(403, 'forbidden', 'No member profile is linked to this account');
        }
    ...
    public static function assertWrite(string $table, array $row, Identity $identity, string $action): void
    {
        ...
        if (($rules['ownWrite'] ?? false) !== true) {
            throw new ApiException(403, 'forbidden', sprintf('You are not allowed to %s "%s"', $action, $table));
        }
        $own = $rules['own'] ?? null;
        if ($own === 'user_id') {
            if (($row['user_id'] ?? null) !== $identity->userId) {
                throw new ApiException(403, 'forbidden', 'You can only modify your own records');
            }
            return;
        }
        if ($own === 'member_id') {
