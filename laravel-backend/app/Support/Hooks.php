...
final class Hooks
{
    ...
    {
        if (in_array('updated_at', Db::columns($table), true)) {
    ...
    {
        $year = gmdate('Y');
        $count = (int) Db::value('SELECT COUNT(*) FROM memos WHERE YEAR(created_at) = ?', [$year]);
    ...
    {
        $exists = (int) Db::value(sprintf('SELECT COUNT(*) FROM `%s`', $wallet));
        if ($exists === 0) {
            Db::run(sprintf('INSERT INTO `%s` (id) VALUES (?)', $wallet), [Uuid::v4()]);
        }
    }

    private static function beneficiaryNotifications(array $row, ?array $previous): void
    {
        $member = Db::one('SELECT name, user_id FROM members WHERE id = ?', [$row['member_id'] ?? '']);
        $memberName = $member['name'] ?? 'A member';

        if ($previous === null) {
            $admins = Db::all("SELECT DISTINCT user_id FROM user_roles WHERE role IN ('admin','super_admin')");
