...
/**
 * Translates between the JSON shapes the React app expects and the way
 * MariaDB stores the same data.
 */
final class Casts
{
    ...
    public static function in(mixed $value): mixed
    {
        if (is_bool($value)) {
            return $value ? 1 : 0;
        }
    ...
    public static function payload(string $table, array $payload): array
    {
        ...
        foreach ($payload as $key => $value) {
            if (! in_array($key, $columns, true)) {
                continue; // drop unknown columns, like PostgREST with a strict schema
            }
